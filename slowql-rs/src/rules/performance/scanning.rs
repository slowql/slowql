...
impl Rule for MissingWhereRule {
    fn id(&self) -> &'static str { "PERF-SCAN-002" }
    fn name(&self) -> &'static str { "Unbounded Data Modification" }
    fn severity(&self) -> Severity { Severity::High }
    fn dimension(&self) -> Dimension { Dimension::Performance }
    fn category(&self) -> Option<Category> { Some(Category::PerfScan) }
    fn impact(&self) -> &'static str { "Will modify/delete ALL rows in the table, causing massive lock contention and log growth." }
    fn check(&self, query: &Query) -> Vec<Issue> {
...
impl Rule for UnboundedSelectRule {
    fn id(&self) -> &'static str { "PERF-SCAN-003" }
    fn name(&self) -> &'static str { "Unbounded SELECT" }
...
static PAT_NOT_IN_SUB: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bNOT\s+IN\s*\(\s*SELECT\b").unwrap());
impl Rule for NotInSubqueryRule {
    fn id(&self) -> &'static str { "PERF-SCAN-004" }
    fn name(&self) -> &'static str { "NOT IN Subquery" }
    fn severity(&self) -> Severity { Severity::Medium }
    fn dimension(&self) -> Dimension { Dimension::Performance }
    fn category(&self) -> Option<Category> { Some(Category::PerfScan) }
    fn impact(&self) -> &'static str { "NOT IN with subquery fails silently with NULLs and disables index usage." }
    fn check(&self, query: &Query) -> Vec<Issue> {
        PAT_NOT_IN_SUB.find(&query.raw).map(|m| {
            vec![self.build_issue(query, "NOT IN with subquery detected. Vulnerable to NULL semantics.", m.as_str())]
        }).unwrap_or_default()
    }
}
...
impl Rule for DistinctOnLargeSetRule {
    fn id(&self) -> &'static str { "PERF-SCAN-005" }
    fn name(&self) -> &'static str { "Expensive DISTINCT" }
    fn severity(&self) -> Severity { Severity::Low }
    fn dimension(&self) -> Dimension { Dimension::Performance }
    fn category(&self) -> Option<Category> { Some(Category::PerfScan) }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        PAT_DISTINCT.find(&query.raw).map(|m| {
            vec![self.build_issue(query, "DISTINCT usage detected. Ensure this is necessary.", m.as_str())]
        }).unwrap_or_default()
    }
}
...
impl Rule for CountStarWithoutWhereRule {
    fn id(&self) -> &'static str { "PERF-PG-002" }
    fn name(&self) -> &'static str { "Unfiltered COUNT(*)" }
    fn severity(&self) -> Severity { Severity::Info }
    fn dimension(&self) -> Dimension { Dimension::Performance }
    fn category(&self) -> Option<Category> { Some(Category::PerfScan) }
    fn dialects(&self) -> DialectSet { DialectSet::new(&["postgresql"]) }
    fn impact(&self) -> &'static str { "COUNT(*) on a large table without WHERE scans every row." }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) { return Vec::new(); }
        if !query.is_select() { return Vec::new(); }
        let upper = query.raw_upper();
        if !upper.contains("COUNT(*)") && !upper.contains("COUNT( *)") { return Vec::new(); }
        if upper.contains("WHERE") { return Vec::new(); }
        let snip = &query.raw[..query.raw.len().min(80)];
        vec![self.build_issue(query, "COUNT(*) without WHERE - consider pg_catalog.reltuples for approximate counts.", snip)]
    }
}
...
impl Rule for NotInNullableSubqueryRule {
    fn id(&self) -> &'static str { "PERF-PG-003" }
    fn name(&self) -> &'static str { "NOT IN With Potentially NULLable Subquery" }
    fn severity(&self) -> Severity { Severity::Medium }
    fn dimension(&self) -> Dimension { Dimension::Performance }
    fn category(&self) -> Option<Category> { Some(Category::PerfScan) }
    fn dialects(&self) -> DialectSet { DialectSet::new(&["postgresql"]) }
    fn impact(&self) -> &'static str { "A single NULL in the subquery result causes NOT IN to return zero rows." }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) { return Vec::new(); }
        if !PAT_NOT_IN_SUB.is_match(&query.raw) { return Vec::new(); }
        let snip = &query.raw[..query.raw.len().min(80)];
...
impl Rule for SelectForUpdateWithoutNowaitPgRule {
    fn id(&self) -> &'static str { "PERF-PG-004" }
    fn name(&self) -> &'static str { "SELECT FOR UPDATE Without NOWAIT/SKIP LOCKED (PG)" }
    fn severity(&self) -> Severity { Severity::Low }
    fn dimension(&self) -> Dimension { Dimension::Performance }
    fn category(&self) -> Option<Category> { Some(Category::PerfLock) }
    fn dialects(&self) -> DialectSet { DialectSet::new(&["postgresql"]) }
    fn impact(&self) -> &'static str { "Without NOWAIT, the query blocks until the lock is released." }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) { return Vec::new(); }
        let upper = query.raw_upper();
        if !upper.contains("FOR UPDATE") { return Vec::new(); }
        if upper.contains("NOWAIT") || upper.contains("SKIP LOCKED") { return Vec::new(); }
        let snip = &query.raw[..query.raw.len().min(80)];
...
impl Rule for SelectForUpdateWithoutLimitMysqlRule {
    fn id(&self) -> &'static str { "PERF-MYSQL-001" }
    fn name(&self) -> &'static str { "SELECT FOR UPDATE Without LIMIT (MySQL)" }
    fn severity(&self) -> Severity { Severity::Medium }
    fn dimension(&self) -> Dimension { Dimension::Performance }
    fn category(&self) -> Option<Category> { Some(Category::PerfLock) }
    fn dialects(&self) -> DialectSet { DialectSet::new(&["mysql"]) }
    fn impact(&self) -> &'static str { "Locking too many rows blocks concurrent writes." }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) { return Vec::new(); }
        let upper = query.raw_upper();
        if !upper.contains("FOR UPDATE") { return Vec::new(); }
        if upper.contains("LIMIT") { return Vec::new(); }
        let snip = &query.raw[..query.raw.len().min(80)];
        vec![self.build_issue(query, "SELECT FOR UPDATE without LIMIT - may lock excessive rows in InnoDB.", snip)]
    }
}

// PERF-MYSQL-002: ORDER BY RAND()
struct OrderByRandRule;
static PAT_RAND: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bORDER\s+BY\s+RAND\s*\(\s*\)").unwrap());
impl Rule for OrderByRandRule {
    fn id(&self) -> &'static str { "PERF-MYSQL-002" }
    fn name(&self) -> &'static str { "ORDER BY RAND() Full Table Sort" }
    fn severity(&self) -> Severity { Severity::High }
    fn dimension(&self) -> Dimension { Dimension::Performance }
    fn category(&self) -> Option<Category> { Some(Category::PerfSort) }
    fn dialects(&self) -> DialectSet { DialectSet::new(&["mysql"]) }
    fn impact(&self) -> &'static str { "On 1M rows, ORDER BY RAND() LIMIT 1 still reads and sorts all 1M rows." }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) { return Vec::new(); }
        PAT_RAND.find(&query.raw).map(|m| {
            vec![self.build_issue(query, "ORDER BY RAND() detected - full table sort regardless of LIMIT.", m.as_str())]
        }).unwrap_or_default()
    }
}
...
static PAT_FORCE_IDX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\b(?:FORCE|USE|IGNORE)\s+INDEX\s*\(").unwrap());
impl Rule for ForceIndexHintMysqlRule {
    fn id(&self) -> &'static str { "PERF-MYSQL-003" }
    fn name(&self) -> &'static str { "FORCE INDEX / USE INDEX Hint" }
    fn severity(&self) -> Severity { Severity::Low }
    fn dimension(&self) -> Dimension { Dimension::Performance }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        PAT_FORCE_IDX.find(&query.raw).map(|m| {
            vec![self.build_issue(query, "Index hint detected - may become suboptimal as data changes.", m.as_str())]
        }).unwrap_or_default()
    }
}
...
static PAT_BQ_UNNEST: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bSELECT\s+DISTINCT\b.*\bUNNEST\s*\(").unwrap());
impl Rule for BigQueryDistinctOnUnnestRule {
    fn id(&self) -> &'static str { "PERF-BQ-001" }
    fn name(&self) -> &'static str { "SELECT DISTINCT on UNNEST" }
    fn severity(&self) -> Severity { Severity::Medium }
    fn dimension(&self) -> Dimension { Dimension::Performance }
    fn category(&self) -> Option<Category> { Some(Category::PerfScan) }
    fn dialects(&self) -> DialectSet { DialectSet::new(&["bigquery"]) }
    fn impact(&self) -> &'static str { "UNNEST explodes arrays then DISTINCT shuffles all rows to deduplicate." }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) { return Vec::new(); }
        PAT_BQ_UNNEST.find(&query.raw).map(|m| {
            vec![self.build_issue(query, "DISTINCT on UNNEST - expensive full shuffle.", m.as_str())]
        }).unwrap_or_default()
    }
}
...
static PAT_BQ_REGEX: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bREGEXP_(?:CONTAINS|EXTRACT|REPLACE)\s*\(").unwrap());
impl Rule for BigQueryRegexOnLargeTableRule {
    fn id(&self) -> &'static str { "PERF-BQ-002" }
    fn name(&self) -> &'static str { "REGEXP on Large Table Without Filter" }
    fn severity(&self) -> Severity { Severity::Low }
    fn dimension(&self) -> Dimension { Dimension::Performance }
    fn category(&self) -> Option<Category> { Some(Category::PerfScan) }
    fn dialects(&self) -> DialectSet { DialectSet::new(&["bigquery"]) }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        PAT_BQ_REGEX.find(&query.raw).map(|m| {
            vec![self.build_issue(query, "REGEXP function detected - ensure WHERE clause limits scan.", m.as_str())]
        }).unwrap_or_default()
    }
}
