...
impl Rule for SqlInjectionRule {
    fn id(&self) -> &'static str { "SEC-INJ-001" }
    fn name(&self) -> &'static str { "Potential SQL Injection" }
    fn severity(&self) -> Severity { Severity::Critical }
    fn dimension(&self) -> Dimension { Dimension::Security }
    fn category(&self) -> Option<Category> { Some(Category::SecInjection) }
    fn impact(&self) -> &'static str { "Using string interpolation to build SQL queries is a primary source of SQL injection vulnerabilities." }
    fn check(&self, query: &Query) -> Vec<Issue> {
        let mut issues = Vec::new();
        if let Some(m) = PAT_INJ_001.find(&query.raw) {
            let msg = format!("Potential SQL injection: string concatenation with variable '{}'", m.as_str());
            issues.push(self.build_issue(query, &msg, m.as_str())
                .with_fix(Fix::guidance("Use parameterized queries instead of string interpolation.", self.id())));
        } else if query.is_dynamic {
            let snip = &query.raw[..query.raw.len().min(100)];
            issues.push(self.build_issue(query, "Potential SQL injection: query is dynamically constructed.", snip)
                .with_fix(Fix::guidance("Use parameterized queries instead of string interpolation.", self.id())));
        }
        issues
    }
...
impl Rule for DynamicSqlExecutionRule {
    fn id(&self) -> &'static str { "SEC-INJ-002" }
    fn name(&self) -> &'static str { "Dynamic SQL Execution" }
...
impl Rule for TautologicalOrConditionRule {
    fn id(&self) -> &'static str { "SEC-INJ-003" }
    fn name(&self) -> &'static str { "Tautological OR Condition" }
    fn severity(&self) -> Severity { Severity::Medium }
    fn dimension(&self) -> Dimension { Dimension::Security }
    fn category(&self) -> Option<Category> { Some(Category::SecInjection) }
    fn impact(&self) -> &'static str { "Tautological OR conditions bypass authentication and authorization checks." }
    fn check(&self, query: &Query) -> Vec<Issue> {
        PAT_INJ_003.find(&query.raw).map(|m| {
...
static PAT_INJ_004: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)(\bWAITFOR\s+DELAY\b)|(\bSLEEP\s*\()|(\bpg_sleep\s*\()|(\bBENCHMARK\s*\()").unwrap()
});
impl Rule for TimeBasedBlindInjectionRule {
    fn id(&self) -> &'static str { "SEC-INJ-004" }
    fn name(&self) -> &'static str { "Time-Based Blind Injection Indicator" }
    fn severity(&self) -> Severity { Severity::High }
    fn dimension(&self) -> Dimension { Dimension::Security }
    fn category(&self) -> Option<Category> { Some(Category::SecInjection) }
    fn impact(&self) -> &'static str { "Blind SQL injection allows attackers to extract data one bit at a time by measuring response delays." }
    fn check(&self, query: &Query) -> Vec<Issue> {
        PAT_INJ_004.find(&query.raw).map(|m| {
...
impl Rule for LdapInjectionRule {
    fn id(&self) -> &'static str { "SEC-INJ-007" }
    fn name(&self) -> &'static str { "LDAP Injection in Directory Queries" }
    fn severity(&self) -> Severity { Severity::High }
    fn dimension(&self) -> Dimension { Dimension::Security }
    fn category(&self) -> Option<Category> { Some(Category::SecInjection) }
    fn check(&self, query: &Query) -> Vec<Issue> {
        PAT_INJ_007.find(&query.raw).map(|m| {
...
impl Rule for NoSqlInjectionRule {
    fn id(&self) -> &'static str { "SEC-INJ-008" }
    fn name(&self) -> &'static str { "NoSQL Injection Pattern" }
    fn severity(&self) -> Severity { Severity::High }
    fn dimension(&self) -> Dimension { Dimension::Security }
    fn category(&self) -> Option<Category> { Some(Category::SecInjection) }
    fn check(&self, query: &Query) -> Vec<Issue> {
        PAT_INJ_008.find(&query.raw).map(|m| {
...
static PAT_INJ_009: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\b(XMLQUERY|XMLEXISTS|XPATH|XQUERY|xml_)\b[^;]*(\+|CONCAT|\|\|)[^;]*[/\[\]]").unwrap()
});
impl Rule for XmlXPathInjectionRule {
    fn id(&self) -> &'static str { "SEC-INJ-009" }
    fn name(&self) -> &'static str { "XML/XPath Injection" }
    fn severity(&self) -> Severity { Severity::High }
    fn dimension(&self) -> Dimension { Dimension::Security }
    fn category(&self) -> Option<Category> { Some(Category::SecInjection) }
    fn check(&self, query: &Query) -> Vec<Issue> {
        PAT_INJ_009.find(&query.raw).map(|m| {
...
static PAT_INJ_010: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\b(RENDER|TEMPLATE|EVAL|EXECUTE|PROCESS|render_)\w*\b\([^)]*(\+|CONCAT|\|\|)").unwrap()
});
impl Rule for ServerSideTemplateInjectionRule {
    fn id(&self) -> &'static str { "SEC-INJ-010" }
    fn name(&self) -> &'static str { "Server-Side Template Injection" }
    fn severity(&self) -> Severity { Severity::High }
    fn dimension(&self) -> Dimension { Dimension::Security }
    fn category(&self) -> Option<Category> { Some(Category::SecInjection) }
    fn check(&self, query: &Query) -> Vec<Issue> {
        PAT_INJ_010.find(&query.raw).map(|m| {
...
impl Rule for JsonFunctionInjectionRule {
    fn id(&self) -> &'static str { "SEC-INJ-011" }
    fn name(&self) -> &'static str { "SQL Injection via JSON Functions" }
    fn severity(&self) -> Severity { Severity::High }
    fn dimension(&self) -> Dimension { Dimension::Security }
    fn category(&self) -> Option<Category> { Some(Category::SecInjection) }
    fn check(&self, query: &Query) -> Vec<Issue> {
        PAT_INJ_011.find(&query.raw).map(|m| {
...
static PAT_PG_003: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\bRAISE\s+(?:NOTICE|WARNING|INFO|LOG|DEBUG)\s+.*\|\|").unwrap()
});
impl Rule for RaiseNoticeInjectionRule {
    fn id(&self) -> &'static str { "SEC-PG-003" }
    fn name(&self) -> &'static str { "RAISE NOTICE Log Injection" }
    fn severity(&self) -> Severity { Severity::Medium }
    fn dimension(&self) -> Dimension { Dimension::Security }
    fn category(&self) -> Option<Category> { Some(Category::SecInjection) }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        PAT_PG_003.find(&query.raw).map(|m| {
            let msg = format!("RAISE NOTICE with concatenation - log injection risk: {}", m.as_str());
            vec![self.build_issue(query, &msg, m.as_str())]
        }).unwrap_or_default()
    }
}
...
static PAT_ORA_002: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\bDBMS_SQL\s*\.\s*(?:PARSE|EXECUTE|OPEN_CURSOR)\b").unwrap()
});
impl Rule for OracleDbmsSqlInjectionRule {
    fn id(&self) -> &'static str { "SEC-ORA-002" }
    fn name(&self) -> &'static str { "DBMS_SQL Dynamic Execution" }
    fn severity(&self) -> Severity { Severity::High }
    fn dimension(&self) -> Dimension { Dimension::Security }
    fn category(&self) -> Option<Category> { Some(Category::SecInjection) }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) { return Vec::new(); }
        PAT_ORA_002.find(&query.raw).map(|m| {
...
static PAT_ORA_003: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\bEXECUTE\s+IMMEDIATE\s+.*\|\|").unwrap()
});
impl Rule for OracleExecuteImmediateConcatRule {
    fn id(&self) -> &'static str { "SEC-ORA-003" }
    fn name(&self) -> &'static str { "EXECUTE IMMEDIATE With Concatenation" }
    fn severity(&self) -> Severity { Severity::Critical }
    fn dimension(&self) -> Dimension { Dimension::Security }
    fn category(&self) -> Option<Category> { Some(Category::SecInjection) }
    fn dialects(&self) -> DialectSet { DialectSet::new(&["oracle"]) }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) { return Vec::new(); }
        PAT_ORA_003.find(&query.raw).map(|m| {
            let msg = format!("EXECUTE IMMEDIATE with concatenation - SQL injection risk: {}", m.as_str());
            vec![self.build_issue(query, &msg, m.as_str())]
        }).unwrap_or_default()
    }
}
