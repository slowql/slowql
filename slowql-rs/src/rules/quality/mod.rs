...
impl Rule for SelectWithoutFromRule {
    fn id(&self) -> &'static str { "QUAL-STYLE-001" }
    fn name(&self) -> &'static str { "SELECT Without FROM Clause" }
...
static PAT_NO_ALIAS: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bFROM\s*\(\s*SELECT\b[^)]+\)\s*WHERE\b").unwrap());
impl Rule for MissingAliasRule { fn id(&self) -> &'static str { "QUAL-STYLE-003" } fn name(&self) -> &'static str { "Subquery Missing Alias" } fn severity(&self) -> Severity { Severity::High } fn dimension(&self) -> Dimension { Dimension::Quality } fn category(&self) -> Option<Category> { Some(Category::QualReadability) } fn impact(&self) -> &'static str { "Unaliased subqueries cause syntax errors in PostgreSQL and MySQL." } fn check(&self, query: &Query) -> Vec<Issue> { PAT_NO_ALIAS.find(&query.raw).map(|m| vec![self.build_issue(query, "Subquery in FROM without alias detected.", m.as_str())]).unwrap_or_default() } }
...
static PAT_IMPLICIT_JOIN: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)FROM\s+\w+\s*,\s*\w+").unwrap());
impl Rule for ImplicitJoinRule {
    fn id(&self) -> &'static str { "QUAL-MODERN-001" }
    fn name(&self) -> &'static str { "Implicit Join Syntax" }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        if PAT_IMPLICIT_JOIN.is_match(&query.raw) && !query.raw_upper().contains("JOIN") {
            return vec![self.build_issue(query, "Implicit join syntax detected (comma-separated tables).", query.snippet(80))];
        }
        Vec::new()
    }
}
...
impl Rule for UnionWithoutAllRule { fn id(&self) -> &'static str { "QUAL-MODERN-003" } fn name(&self) -> &'static str { "UNION Without ALL" } fn severity(&self) -> Severity { Severity::Low } fn dimension(&self) -> Dimension { Dimension::Quality } fn category(&self) -> Option<Category> { Some(Category::QualModern) } fn impact(&self) -> &'static str { "UNION deduplicates using expensive sort or hash." }
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        if upper.contains("UNION ALL") { return Vec::new(); }
        if upper.contains("UNION") { return vec![self.build_issue(query, "UNION without ALL - use UNION ALL if duplicates are not a concern.", query.snippet(80))]; }
        Vec::new()
    }
}
...
impl Rule for CaseWithoutElseRule { fn id(&self) -> &'static str { "QUAL-MODERN-004" } fn name(&self) -> &'static str { "CASE Without ELSE" } fn severity(&self) -> Severity { Severity::Low } fn dimension(&self) -> Dimension { Dimension::Quality } fn category(&self) -> Option<Category> { Some(Category::QualReadability) } fn impact(&self) -> &'static str { "Unmatched CASE returns NULL, which propagates through calculations." } fn check(&self, query: &Query) -> Vec<Issue> { let upper = query.raw_upper(); if !upper.contains("CASE") || !upper.contains("WHEN") { return Vec::new(); } if upper.contains("ELSE") { return Vec::new(); } vec![self.build_issue(query, "CASE without ELSE - returns NULL when no condition matches.", query.snippet(80))] } }
...
impl Rule for DuplicateConditionRule { fn id(&self) -> &'static str { "QUAL-DRY-001" } fn name(&self) -> &'static str { "Duplicate WHERE Condition" } fn severity(&self) -> Severity { Severity::Medium } fn dimension(&self) -> Dimension { Dimension::Quality } fn category(&self) -> Option<Category> { Some(Category::QualDry) } fn impact(&self) -> &'static str { "Duplicate conditions waste parser cycles and obscure intent." } fn check(&self, query: &Query) -> Vec<Issue> { {
        ...
        for i in 0..parts.len() {
            for j in (i+1)..parts.len() {
                ...
                let b = parts[j].trim();
                if !a.is_empty() && a == b {
                    return vec![self.build_issue(query, "Duplicate WHERE condition detected - possible copy-paste error.", query.snippet(80))];
                }
            }
        }
        Vec::new()
...
impl Rule for AmbiguousAliasRule {
    fn id(&self) -> &'static str { "QUAL-NAME-002" }
    fn name(&self) -> &'static str { "Ambiguous Alias" }
    fn severity(&self) -> Severity { Severity::Low }
    fn dimension(&self) -> Dimension { Dimension::Quality }
...
static PAT_CREATE_NO_COMMENT: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)CREATE\s+TABLE\b").unwrap());
impl Rule for MissingColumnCommentsRule {
    fn id(&self) -> &'static str { "QUAL-DOC-001" }
    fn name(&self) -> &'static str { "Missing Column Comments" }
    fn severity(&self) -> Severity { Severity::Info }
    fn dimension(&self) -> Dimension { Dimension::Quality }
...
impl Rule for MissingPrimaryKeyRule {
    fn id(&self) -> &'static str { "QUAL-SCHEMA-001" }
    fn name(&self) -> &'static str { "Missing Primary Key" }
...
impl Rule for MissingForeignKeyRule {
    fn id(&self) -> &'static str { "QUAL-SCHEMA-002" }
    fn name(&self) -> &'static str { "Implicit Foreign Key (Logic)" }
    fn severity(&self) -> Severity { Severity::Medium }
    fn dimension(&self) -> Dimension { Dimension::Quality }
    fn category(&self) -> Option<Category> { Some(Category::QualSchemaDesign) }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        vec![self.build_issue(query, "Column with *_id pattern missing FOREIGN KEY constraint.", query.snippet(100))]
    }
}
...
impl Rule for UsingFloatForCurrencyRule { fn id(&self) -> &'static str { "QUAL-SCHEMA-004" } fn name(&self) -> &'static str { "Float for Currency" } fn severity(&self) -> Severity { Severity::High } fn dimension(&self) -> Dimension { Dimension::Quality } fn category(&self) -> Option<Category> { Some(Category::QualSchemaDesign) } fn impact(&self) -> &'static str { "Float types lead to rounding errors catastrophic for financial data." } fn check(&self, query: &Query) -> Vec<Issue> { PAT_FLOAT_MONEY.find(&query.raw).map(|m| vec![self.build_issue(query, "Float/Double type for currency - use DECIMAL or NUMERIC.", m.as_str())]).unwrap_or_default() } }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        if upper.contains("PROCEDURE") || upper.contains("FUNCTION") {
            if let Some(ret_pos) = upper.find("RETURN") {
                ...
                if after.contains("SELECT") || after.contains("UPDATE") || after.contains("INSERT") || after.contains("DELETE") {
                    return vec![self.build_issue(query, "Unreachable code after RETURN statement.", query.snippet(100))];
                }
            }
        }
        Vec::new()
...
impl Rule for RownumWithoutOrderByRule { fn id(&self) -> &'static str { "QUAL-ORA-001" } fn name(&self) -> &'static str { "ROWNUM Without ORDER BY" } fn severity(&self) -> Severity { Severity::High } fn dimension(&self) -> Dimension { Dimension::Quality } fn category(&self) -> Option<Category> { Some(Category::QualReadability) } fn dialects(&self) -> DialectSet { DialectSet::new(&["oracle"]) } fn impact(&self) -> &'static str { "ROWNUM filters rows BEFORE ORDER BY is applied." } fn check(&self, query: &Query) -> Vec<Issue> { if !self.dialect_matches(query) { return Vec::new(); } if PAT_ROWNUM.is_match(&query.raw) && !query.raw_upper().contains("ORDER BY") { return vec![self.build_issue(query, "ROWNUM used without ORDER BY - non-deterministic.", query.snippet(80))]; } Vec::new() } }
...
impl Rule for OracleNvlInWhereRule { fn id(&self) -> &'static str { "QUAL-ORA-003" } fn name(&self) -> &'static str { "NVL in WHERE Clause" } fn severity(&self) -> Severity { Severity::Low } fn dimension(&self) -> Dimension { Dimension::Quality } fn category(&self) -> Option<Category> { Some(Category::QualModern) } fn dialects(&self) -> DialectSet { DialectSet::new(&["oracle"]) } fn impact(&self) -> &'static str { "NVL() in WHERE makes the predicate non-SARGable." } fn check(&self, query: &Query) -> Vec<Issue> { if !self.dialect_matches(query) { return Vec::new(); } PAT_NVL.find(&query.raw).map(|m| vec![self.build_issue(query, "NVL() in WHERE clause - prevents index usage.", m.as_str())]).unwrap_or_default() } }

struct SqlCalcFoundRowsRule;
static PAT_CALC: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bSQL_CALC_FOUND_ROWS\b").unwrap());
impl Rule for SqlCalcFoundRowsRule { fn id(&self) -> &'static str { "QUAL-MYSQL-001" } fn name(&self) -> &'static str { "Deprecated SQL_CALC_FOUND_ROWS" } fn severity(&self) -> Severity { Severity::Low } fn dimension(&self) -> Dimension { Dimension::Quality } fn category(&self) -> Option<Category> { Some(Category::QualModern) } fn dialects(&self) -> DialectSet { DialectSet::new(&["mysql"]) } fn impact(&self) -> &'static str { "SQL_CALC_FOUND_ROWS disables LIMIT optimisations." } fn check(&self, query: &Query) -> Vec<Issue> { if !self.dialect_matches(query) { return Vec::new(); } PAT_CALC.find(&query.raw).map(|m| vec![self.build_issue(query, "Deprecated SQL_CALC_FOUND_ROWS usage.", m.as_str())]).unwrap_or_default() } }
...
impl Rule for MysqlLockInShareModeRule { fn id(&self) -> &'static str { "QUAL-MYSQL-003" } fn name(&self) -> &'static str { "Deprecated LOCK IN SHARE MODE" } fn severity(&self) -> Severity { Severity::Low } fn dimension(&self) -> Dimension { Dimension::Quality } fn category(&self) -> Option<Category> { Some(Category::QualModern) } fn dialects(&self) -> DialectSet { DialectSet::new(&["mysql"]) } fn impact(&self) -> &'static str { "LOCK IN SHARE MODE will break in future MySQL versions." } fn check(&self, query: &Query) -> Vec<Issue> { if !self.dialect_matches(query) { return Vec::new(); } PAT_LOCK_SHARE.find(&query.raw).map(|m| vec![self.build_issue(query, "Deprecated LOCK IN SHARE MODE - use FOR SHARE.", m.as_str())]).unwrap_or_default() } }

struct AnsiNullsOffRule;
static PAT_ANSI: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bSET\s+ANSI_NULLS\s+OFF\b").unwrap());
impl Rule for AnsiNullsOffRule { fn id(&self) -> &'static str { "QUAL-TSQL-001" } fn name(&self) -> &'static str { "SET ANSI_NULLS OFF" } fn severity(&self) -> Severity { Severity::Medium } fn dimension(&self) -> Dimension { Dimension::Quality } fn category(&self) -> Option<Category> { Some(Category::QualModern) } fn dialects(&self) -> DialectSet { DialectSet::new(&["tsql"]) } fn impact(&self) -> &'static str { "Code relying on ANSI_NULLS OFF will break when removed." } fn check(&self, query: &Query) -> Vec<Issue> { if !self.dialect_matches(query) { return Vec::new(); } PAT_ANSI.find(&query.raw).map(|m| vec![self.build_issue(query, "SET ANSI_NULLS OFF detected - deprecated non-standard behavior.", m.as_str())]).unwrap_or_default() } }

struct TsqlQuotedIdentifierOffRule;
static PAT_QUOTED: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bSET\s+QUOTED_IDENTIFIER\s+OFF\b").unwrap());
impl Rule for TsqlQuotedIdentifierOffRule { fn id(&self) -> &'static str { "QUAL-TSQL-002" } fn name(&self) -> &'static str { "SET QUOTED_IDENTIFIER OFF" } fn severity(&self) -> Severity { Severity::Medium } fn dimension(&self) -> Dimension { Dimension::Quality } fn category(&self) -> Option<Category> { Some(Category::QualModern) } fn dialects(&self) -> DialectSet { DialectSet::new(&["tsql"]) } fn impact(&self) -> &'static str { "QUOTED_IDENTIFIER OFF breaks indexed views and computed columns." } fn check(&self, query: &Query) -> Vec<Issue> { if !self.dialect_matches(query) { return Vec::new(); } PAT_QUOTED.find(&query.raw).map(|m| vec![self.build_issue(query, "SET QUOTED_IDENTIFIER OFF - deprecated, breaks indexes.", m.as_str())]).unwrap_or_default() } }

struct PgDoBlockWithoutLanguageRule;
static PAT_DO_BLOCK: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?is)\bDO\s+\$\$").unwrap());
impl Rule for PgDoBlockWithoutLanguageRule { fn id(&self) -> &'static str { "QUAL-PG-001" } fn name(&self) -> &'static str { "DO Block Without LANGUAGE" } fn severity(&self) -> Severity { Severity::Low } fn dimension(&self) -> Dimension { Dimension::Quality } fn category(&self) -> Option<Category> { Some(Category::QualReadability) } fn dialects(&self) -> DialectSet { DialectSet::new(&["postgresql"]) } fn impact(&self) -> &'static str { "Implicit language defaults reduce code clarity." } fn check(&self, query: &Query) -> Vec<Issue> { if !self.dialect_matches(query) { return Vec::new(); } if PAT_DO_BLOCK.is_match(&query.raw) && !query.raw_upper().contains("LANGUAGE") { return vec![self.build_issue(query, "DO block without LANGUAGE specification.", query.snippet(80))]; } Vec::new() } }

struct RedshiftDiststyleAllRule;
static PAT_DIST_ALL: Lazy<Regex> = Lazy::new(|| Regex::new(r"(?i)\bDISTSTYLE\s+ALL\b").unwrap());
impl Rule for RedshiftDiststyleAllRule { fn id(&self) -> &'static str { "QUAL-RS-001" } fn name(&self) -> &'static str { "DISTSTYLE ALL on Large Table" } fn severity(&self) -> Severity { Severity::Medium } fn dimension(&self) -> Dimension { Dimension::Quality } fn category(&self) -> Option<Category> { Some(Category::QualSchemaDesign) } fn dialects(&self) -> DialectSet { DialectSet::new(&["redshift"]) } fn impact(&self) -> &'static str { "Entire table copied to every node. Only for small dimension tables." } fn check(&self, query: &Query) -> Vec<Issue> { if !self.dialect_matches(query) { return Vec::new(); } PAT_DIST_ALL.find(&query.raw).map(|m| vec![self.build_issue(query, "DISTSTYLE ALL detected - entire table copied to every node.", m.as_str())]).unwrap_or_default() } }

struct ClickHouseOrderByWithoutLimitRule;
impl Rule for ClickHouseOrderByWithoutLimitRule { fn id(&self) -> &'static str { "QUAL-CH-001" } fn name(&self) -> &'static str { "ORDER BY Without LIMIT on ClickHouse" } fn severity(&self) -> Severity { Severity::Medium } fn dimension(&self) -> Dimension { Dimension::Quality } fn category(&self) -> Option<Category> { Some(Category::QualModern) } fn dialects(&self) -> DialectSet { DialectSet::new(&["clickhouse"]) } fn impact(&self) -> &'static str { "All data gathered to one node for global sorting." } fn check(&self, query: &Query) -> Vec<Issue> { if !self.dialect_matches(query) { return Vec::new(); } if !query.is_select() { return Vec::new(); } let upper = query.raw_upper(); if upper.contains("ORDER BY") && !upper.contains("LIMIT") { return vec![self.build_issue(query, "ORDER BY without LIMIT on ClickHouse - full sort on single node.", query.snippet(80))]; } Vec::new() } }
...
pub fn all_rules() -> Vec<Box<dyn Rule>> {
    vec![
        Box::new(SelectWithoutFromRule), Box::new(WildcardInColumnListRule), Box::new(MissingAliasRule),
        Box::new(CommentedCodeRule), Box::new(InsertWithoutColumnListRule),
        Box::new(NullComparisonRule), Box::new(CaseWithoutElseRule),
        Box::new(ImplicitJoinRule), Box::new(HardcodedDateRule), Box::new(UnionWithoutAllRule),
        Box::new(DuplicateConditionRule),
        Box::new(ExcessiveCaseNestingRule), Box::new(ExcessiveSubqueryNestingRule), Box::new(GodQueryRule),
        Box::new(CyclomaticComplexityRule), Box::new(LongQueryRule),
        Box::new(InconsistentTableNamingRule), Box::new(AmbiguousAliasRule), Box::new(HungarianNotationRule), Box::new(ReservedWordAsColumnRule),
        Box::new(MissingColumnCommentsRule), Box::new(MagicStringWithoutCommentRule), Box::new(ComplexLogicWithoutExplanationRule),
        Box::new(MissingPrimaryKeyRule), Box::new(MissingForeignKeyRule), Box::new(LackOfIndexingOnForeignKeyRule), Box::new(UsingFloatForCurrencyRule),
        Box::new(NonDeterministicQueryRule), Box::new(OrderByMissingForPaginationRule), Box::new(HardcodedTestDataRule),
        Box::new(TodoFixmeCommentRule), Box::new(TempTableNotCleanedUpRule),
        Box::new(UnusedObjectRule), Box::new(UnreachableCodeRule), Box::new(DuplicateQueryRule),
        Box::new(DbtMissingRefRule), Box::new(DbtHardcodedSchemaRule),
        Box::new(JsonbOperatorSpacingRule),
        Box::new(RownumWithoutOrderByRule), Box::new(SelectFromDualRule), Box::new(OracleNvlInWhereRule),
        Box::new(SqlCalcFoundRowsRule), Box::new(StraightJoinHintRule), Box::new(MysqlLockInShareModeRule),
        Box::new(AnsiNullsOffRule), Box::new(TsqlQuotedIdentifierOffRule),
        Box::new(PgDoBlockWithoutLanguageRule), Box::new(RedshiftDiststyleAllRule),
        Box::new(ClickHouseOrderByWithoutLimitRule), Box::new(SnowflakeFlattenWithoutPathRule),
