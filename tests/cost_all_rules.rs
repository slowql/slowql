...
fn compute_001() {
    let r = all();
    let rule = find(&r, "COST-COMPUTE-001");
    assert!(!rule
...
fn compute_002() {
    let r = all();
    let rule = find(&r, "COST-COMPUTE-002");
    assert!(!rule
        .check(&q(
    ...
    assert!(rule
        .check(&q(
            "SELECT ROW_NUMBER() OVER (PARTITION BY id) FROM t",
            "postgresql",
            "SELECT"
        ))
        .is_empty());
...
fn storage_001() {
    let r = all();
    let rule = find(&r, "COST-STORAGE-001");
    assert!(!rule
        .check(&q("INSERT INTO t SELECT * FROM s", "postgresql", "INSERT"))
        .is_empty());
    assert!(rule
        .check(&q("INSERT INTO t SELECT id FROM s", "postgresql", "INSERT"))
        .is_empty());
}
#[test]
fn io_001() {
    let r = all();
    let rule = find(&r, "COST-IO-001");
    assert!(!rule
        .check(&q(
...
fn bq_001() {
    let r = all();
    let rule = find(&r, "COST-BQ-001");
    assert!(!rule
        .check(&q("SELECT * FROM t", "bigquery", "SELECT"))
        .is_empty());
    assert!(rule
...
fn sf_001() {
    let r = all();
    let rule = find(&r, "COST-SF-001");
    assert!(!rule
        .check(&q("SELECT * FROM t", "snowflake", "SELECT"))
        .is_empty());
    assert!(rule
        .check(&q("SELECT * FROM t", "postgresql", "SELECT"))
        .is_empty());
...
fn ch_001() {
    let r = all();
    let rule = find(&r, "COST-CH-001");
    assert!(!rule
        .check(&q("SELECT * FROM t", "clickhouse", "SELECT"))
        .is_empty());
    assert!(rule
        .check(&q("SELECT * FROM t", "postgresql", "SELECT"))
        .is_empty());
...
fn sf_variant() {
    let r = all();
    let rule = find(&r, "PERF-SF-001");
    assert!(!rule
        .check(&q(
            "SELECT * FROM t WHERE data:field = 'x'",
            "snowflake",
            "SELECT"
        ))
        .is_empty());
...
fn rel_sf_001() {
    let r = all();
    let rule = find(&r, "REL-SF-001");
    assert!(!rule
        .check(&q("COPY INTO t FROM @stage", "snowflake", "SELECT"))
        .is_empty());
    assert!(rule
        .check(&q(
            "COPY INTO t FROM @stage ON_ERROR='CONTINUE'",
            "snowflake",
            "SELECT"
        ))
        .is_empty());
...
fn spark_001() {
    let r = all();
    let rule = find(&r, "COST-SPARK-001");
    assert!(!rule
        .check(&q("SELECT id FROM t", "spark", "SELECT"))
        .is_empty());
    assert!(rule
        .check(&q("SELECT id FROM t WHERE dt='2024'", "spark", "SELECT"))
        .is_empty());
}
#[test]
fn tsql_cursor() {
    let r = all();
    let rule = find(&r, "COST-TSQL-001");
    assert!(!rule
        .check(&q(
...
fn partition_001_no_metadata_no_fire() {
    ...
    assert!(rule
        ...
        .is_empty());
    assert!(rule
        .check(&q("SELECT * FROM events", "postgresql", "SELECT"))
        .is_empty());
    assert!(rule
        .check(&q("SELECT * FROM logs", "postgresql", "SELECT"))
        .is_empty());
}
...
fn partition_001_with_context_fires() {
    ...
    let mut query = q(
        "SELECT id, amount FROM transactions WHERE account_id = 42",
        "postgresql",
        "SELECT",
    );
    ...
    assert!(
        !issues.is_empty(),
        "should fire when partition column missing from WHERE"
    );
...
fn partition_001_with_context_no_fire_when_partition_col_present() {
    ...
    let mut query = q(
        "SELECT id, amount FROM transactions WHERE created_at >= '2024-01-01' AND account_id = 42",
        "postgresql",
        "SELECT",
    );
    query.tables = vec!["transactions".to_string()];
    ...
    assert!(
        issues.is_empty(),
        "should not fire when partition column is in WHERE"
    );
}
...
fn partition_001_non_partitioned_table_no_fire() {
    ...
    assert!(
        issues.is_empty(),
        "should not fire on non-partitioned table"
    );
}
...
fn partition_001_schema_partition_metadata() {
    use slowql_lib::config::TableMetadata;
    use slowql_lib::rules::base::RuleContext;
    use slowql_lib::schema::{Column, Schema, Table};
    ...
    assert!(
        !issues.is_empty(),
        "should fire from schema partition metadata"
    );
    ...
    query2.facts = Some(slowql_lib::query_analysis::QueryFacts::from_sql(
        &query2.raw,
        "postgresql",
    ));
