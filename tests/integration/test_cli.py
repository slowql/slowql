...
def run_cli(args, capsys, monkeypatch=None):
    ...
    if monkeypatch:
        ...
        monkeypatch.setattr("slowql.cli.app.show_quick_actions_menu", lambda *a, **k: False)
        # Prevent confirmation prompts (e.g. export session, error continuation)
        monkeypatch.setattr("slowql.cli.app.Confirm.ask", lambda *a, **k: False)
...
def test_cli_empty_file(empty_sql_file, capsys, monkeypatch):
    out, code = run_cli(["--fast", "--no-intro", "--input-file", str(empty_sql_file)], capsys, monkeypatch)
    assert code == 0
...
def test_cli_main_entrypoint(sample_sql_file):
    result = subprocess.run(
        [sys.executable, "-m", "slowql.cli.app", "--no-intro", "--fast", "--non-interactive", "--input-file", str(sample_sql_file)],
