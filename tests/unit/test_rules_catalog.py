...
from slowql.core.models import Query, Location, Severity
...
class TestCatalogRules:
    ...
    def test_grant_all_rule(self):
        ...
        ast = exp.Grant(
            actions=[exp.Literal.string("ALL")]
        )
        query = create_query("GRANT ALL ...", "GRANT")
        
        # Helper to inject 'actions' attribute that sqlglot might not expose directly via __init__ comfortably
        ...
        # Note: The rule checks 'actions' attribute. sqlglot Grant node puts actions in 'actions' arg.
