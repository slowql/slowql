...
impl Rule for ConsentTableMissingRule { fn id(&self) -> &'static str { "COMP-GDPR-004" } fn name(&self) -> &'static str { "Marketing Insert Without Consent Signal" } fn severity(&self) -> Severity { Severity::Medium } fn dimension(&self) -> Dimension { Dimension::Compliance } fn category(&self) -> Option<Category> { Some(Category::CompGdpr) } fn impact(&self) -> &'static str { "Adding users to marketing lists without consent violates GDPR Article 7." } 
    fn confidence(&self) -> RuleConfidence { RuleConfidence::Contextual }
    fn check(&self, query: &Query) -> Vec<Issue> { PAT_MARKETING.find(&query.raw).map(|m| { let msg = format!("INSERT into marketing table - verify GDPR consent: {}", m.as_str()); vec![self.build_issue(query, &msg, m.as_str())] }).unwrap_or_default() } }

// COMP-GDPR-005
struct DataExportCompletenessRule;
impl Rule for DataExportCompletenessRule { fn id(&self) -> &'static str { "COMP-GDPR-005" } fn name(&self) -> &'static str { "Data Subject Request Without Completeness Check" } fn severity(&self) -> Severity { Severity::Medium } fn dimension(&self) -> Dimension { Dimension::Compliance } fn category(&self) -> Option<Category> { Some(Category::CompGdpr) } fn impact(&self) -> &'static str { "Incomplete DSAR responses violate GDPR Article 15." } 
    fn confidence(&self) -> RuleConfidence { RuleConfidence::Contextual }
    fn check(&self, query: &Query) -> Vec<Issue> { if !query.is_select() { return Vec::new(); } let lower = query.raw_lower(); if !lower.contains("export") && !lower.contains("dsar") && !lower.contains("access_request") && !lower.contains("subject_data") { return Vec::new(); } if lower.contains("users") && !lower.contains("activity_log") && !lower.contains("user_log") && !lower.contains("audit_log") { return vec![self.build_issue(query, "User data export might be missing related audit or activity logs.", query.snippet(100))]; } Vec::new() } }
...
impl Rule for ConsentWithdrawalRule { fn id(&self) -> &'static str { "COMP-GDPR-006" } fn name(&self) -> &'static str { "Consent Withdrawal Not Honored" } fn severity(&self) -> Severity { Severity::High } fn dimension(&self) -> Dimension { Dimension::Compliance } fn category(&self) -> Option<Category> { Some(Category::CompGdpr) } fn impact(&self) -> &'static str { "Failing to honor consent withdrawal violates GDPR Article 7." } 
    fn confidence(&self) -> RuleConfidence { RuleConfidence::Contextual }
    fn check(&self, query: &Query) -> Vec<Issue> { if !query.is_select() { return Vec::new(); }
...
impl Rule for PhiAccessWithoutAuditRule { fn id(&self) -> &'static str { "COMP-HIPAA-001" } fn name(&self) -> &'static str { "PHI Access Without Audit Trail" } fn severity(&self) -> Severity { Severity::High } fn dimension(&self) -> Dimension { Dimension::Compliance } fn category(&self) -> Option<Category> { Some(Category::CompHipaa) } fn impact(&self) -> &'static str { "Lack of audit trails for PHI access violates HIPAA Technical Safeguards." } 
    fn confidence(&self) -> RuleConfidence { RuleConfidence::Contextual }
    fn check(&self, query: &Query) -> Vec<Issue> { let lower = query.raw_lower(); let is_phi = PHI_TABLES.iter().any(|t| lower.contains(t)) || PHI_COLS.iter().any(|c| lower.contains(c)); if !is_phi { return Vec::new(); } if lower.contains("audit") || lower.contains("access_log") || lower.contains("phi_log") || lower.contains("compliance_log") { return Vec::new(); } vec![self.build_issue(query, "PHI access detected without apparent audit logging reference.", query.snippet(100))] } }

// COMP-HIPAA-002
struct PhiMinimumNecessaryRule;
impl Rule for PhiMinimumNecessaryRule { fn id(&self) -> &'static str { "COMP-HIPAA-002" } fn name(&self) -> &'static str { "PHI Minimum Necessary Violation" } fn severity(&self) -> Severity { Severity::Medium } fn dimension(&self) -> Dimension { Dimension::Compliance } fn category(&self) -> Option<Category> { Some(Category::CompHipaa) } fn impact(&self) -> &'static str { "Fetching all columns from healthcare tables retrieves unnecessary PHI." } 
    fn confidence(&self) -> RuleConfidence { RuleConfidence::Contextual }
    fn check(&self, query: &Query) -> Vec<Issue> { if !query.is_select() { return Vec::new(); } let upper = query.raw_upper(); if !upper.contains("SELECT *") { return Vec::new(); } let lower = query.raw_lower(); if PHI_TABLES.iter().any(|t| lower.contains(t)) { return vec![self.build_issue(query, "SELECT * on PHI table - violates Minimum Necessary standard.", "SELECT *")]; } Vec::new() } }
...
impl Rule for UnencryptedPhiTransitRule { fn id(&self) -> &'static str { "COMP-HIPAA-003" } fn name(&self) -> &'static str { "Unencrypted PHI Transit Signal" } fn severity(&self) -> Severity { Severity::High } fn dimension(&self) -> Dimension { Dimension::Compliance } fn category(&self) -> Option<Category> { Some(Category::CompHipaa) } fn impact(&self) -> &'static str { "Transmitting PHI over unencrypted connections violates HIPAA Security Rule." } 
    fn confidence(&self) -> RuleConfidence { RuleConfidence::Contextual }
    fn check(&self, query: &Query) -> Vec<Issue> { PAT_UNENC_PHI.find(&query.raw).map(|m| vec![self.build_issue(query, "Insecure connection parameters for PHI-related database.", m.as_str())]).unwrap_or_default() } }
...
impl Rule for UnencryptedSensitiveColumnRule { fn id(&self) -> &'static str { "COMP-SEC-001" } fn name(&self) -> &'static str { "Unencrypted Sensitive Column Definition" } fn severity(&self) -> Severity { Severity::High } fn dimension(&self) -> Dimension { Dimension::Compliance } fn category(&self) -> Option<Category> { Some(Category::CompPci) } fn impact(&self) -> &'static str { "Storing sensitive values in plain text violates PCI-DSS, HIPAA, and GDPR." } 
    fn confidence(&self) -> RuleConfidence { RuleConfidence::Contextual }
    fn check(&self, query: &Query) -> Vec<Issue> { PAT_UNENC_COL.find(&query.raw).map(|m| vec![self.build_issue(query, "Sensitive column defined with plain text type - consider encryption.", m.as_str())]).unwrap_or_default() } }
...
impl Rule for AuditLogTamperingRule { fn id(&self) -> &'static str { "COMP-AUD-001" } fn name(&self) -> &'static str { "Audit Log Tampering Risk" } fn severity(&self) -> Severity { Severity::High } fn dimension(&self) -> Dimension { Dimension::Compliance } fn category(&self) -> Option<Category> { Some(Category::CompSox) } fn impact(&self) -> &'static str { "Modifying audit logs violates regulatory non-repudiation requirements." } 
    fn confidence(&self) -> RuleConfidence { RuleConfidence::Contextual }
    fn check(&self, query: &Query) -> Vec<Issue> { PAT_AUDIT_TAMP.find(&query.raw).map(|m| vec![self.build_issue(query, "Modification of audit/log table detected - potential compliance violation.", m.as_str())]).unwrap_or_default() } }
...
impl Rule for CcpaOptOutRule { fn id(&self) -> &'static str { "COMP-CCPA-001" } fn name(&self) -> &'static str { "Do Not Sell Flag Not Checked" } fn severity(&self) -> Severity { Severity::High } fn dimension(&self) -> Dimension { Dimension::Compliance } fn category(&self) -> Option<Category> { Some(Category::CompCcpa) } fn impact(&self) -> &'static str { "Processing sale of data for opted-out consumers violates CCPA." } 
    fn confidence(&self) -> RuleConfidence { RuleConfidence::Contextual }
    fn check(&self, query: &Query) -> Vec<Issue> { if !query.is_select() { return Vec::new(); } let lower = query.raw_lower(); if !lower.contains("marketing") && !lower.contains("sharing") && !lower.contains("third_party") && !lower.contains("affiliate") { return Vec::new(); } let has_dns = ["do_not_sell","dns_flag","opt_out","ccpa_status"].iter().any(|c| lower.contains(c)); if has_dns { return Vec::new(); } vec![self.build_issue(query, "Data share/sale query without CCPA Do Not Sell flag check.", query.snippet(100))] } }
