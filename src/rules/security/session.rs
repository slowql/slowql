    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        if let Some(m) = PAT_SESSION_002.find(&query.raw) {
            ...
            let has_expiry_check = ["expir", "valid_until", "expires_at", "ttl", "created_at",
                                    "last_activity", "timeout", "max_age"]
                .iter()
                .any(|needle| raw_lower.contains(needle));
            if has_expiry_check { return Vec::new(); }
            ...
            let is_token_lookup = ["session_token", "session_id", "auth_token", "access_token",
                                   "refresh_token", "bearer_token", "token_hash"]
                .iter()
                .any(|needle| raw_lower.contains(needle));
            if !is_token_lookup { return Vec::new(); }
