...
impl Rule for ExcessiveColumnCountRule {
    ...
    fn name(&self) -> &'static str {
        "Excessive Column Count in SELECT"
    }
    ...
    fn category(&self) -> Option<Category> {
        Some(Category::PerfNetwork)
    }
    fn impact(&self) -> &'static str {
        "Wide result sets waste network bandwidth and consume more memory."
    }
...
impl Rule for MissingSetNocountRule {
    ...
    fn impact(&self) -> &'static str {
        "Each DML statement sends a row count message to the client, wasting bandwidth."
    }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) {
            return Vec::new();
        }
        if !PAT_NOCOUNT.is_match(&query.raw) {
            return Vec::new();
        }
...
impl Rule for RedshiftSelectStarRule {
    ...
    fn name(&self) -> &'static str {
        "SELECT * on Redshift Columnar Storage"
    }
    fn severity(&self) -> Severity {
        Severity::High
    }
    ...
    fn impact(&self) -> &'static str {
        "Redshift charges for bytes scanned. SELECT * reads 100x more data than needed."
    }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) {
            return Vec::new();
        ...
        PAT_RS_STAR
            ...
            .map(|m| {
                vec![self.build_issue(
                    query,
                    "SELECT * on Redshift - reads all columns from columnar storage.",
                    m.as_str(),
                )]
            })
            .unwrap_or_default()
...
impl Rule for OrderByWithoutLimitRedshiftRule {
    ...
    fn name(&self) -> &'static str {
        "ORDER BY Without LIMIT on Redshift"
    }
    fn severity(&self) -> Severity {
        Severity::Medium
    }
...
static PAT_RS_NOTIN: Lazy<Regex> =
...
impl Rule for NotInOnRedshiftRule {
    ...
    fn name(&self) -> &'static str {
        "NOT IN on Redshift (Hash Join Explosion)"
    }
    fn severity(&self) -> Severity {
        Severity::Medium
    }
    ...
    fn impact(&self) -> &'static str {
        "NOT IN forces Redshift to build a hash table. With NULLs it degrades to a nested loop."
    }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) {
            return Vec::new();
        ...
        PAT_RS_NOTIN
            ...
            .map(|m| {
                vec![self.build_issue(
                    query,
                    "NOT IN with subquery on Redshift - use NOT EXISTS or LEFT JOIN/NULL.",
                    m.as_str(),
                )]
            })
            .unwrap_or_default()
...
impl Rule for ClickHouseSelectWithoutPrewhereRule {
    ...
    fn impact(&self) -> &'static str {
        "Without PREWHERE, ClickHouse reads all columns from disk before filtering."
    }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        if !PAT_CH_PRE.is_match(&query.raw) {
            return Vec::new();
        }
...
impl Rule for ClickHouseJoinWithoutGlobalRule {
    ...
    fn name(&self) -> &'static str {
        "JOIN Without GLOBAL on Distributed Table"
    }
    fn severity(&self) -> Severity {
        Severity::Medium
    }
    ...
    fn impact(&self) -> &'static str {
        "Without GLOBAL, each shard executes the right-side subquery independently."
    }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        static PAT_CH_JOIN: Lazy<Regex> =
            Lazy::new(|| Regex::new(r"(?i)\bJOIN\s*\(\s*SELECT\b").unwrap());
...
static PAT_CH_MUT: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bALTER\s+TABLE\s+\w+\s+(?:UPDATE|DELETE)\b").unwrap());
impl Rule for ClickHouseMutationRule {
    fn id(&self) -> &'static str {
        "PERF-CH-003"
    }
    fn name(&self) -> &'static str {
        "ClickHouse Mutation (ALTER UPDATE/DELETE)"
    }
    fn severity(&self) -> Severity {
        Severity::Medium
    }
    ...
    fn impact(&self) -> &'static str {
        "Mutations rewrite entire data parts asynchronously. Not designed for frequent modifications."
    }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) {
            return Vec::new();
        ...
        PAT_CH_MUT
            ...
            .map(|m| {
                vec![self.build_issue(
                    query,
                    "ClickHouse mutation detected - heavy async part rewrite.",
                    m.as_str(),
                )]
            })
            .unwrap_or_default()
...
static PAT_PRESTO_CROSS: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bFROM\s+\w+\s*,\s*\w+\b").unwrap());
impl Rule for PrestoCrossJoinRule {
    fn id(&self) -> &'static str {
        "PERF-PRESTO-001"
    }
    fn name(&self) -> &'static str {
        "Implicit Cross-Join on Distributed Engine"
    }
    fn severity(&self) -> Severity {
        Severity::High
    }
    ...
    fn impact(&self) -> &'static str {
        "Cross-joins on distributed engines shuffle all data. Two 1M-row tables produce 1 trillion intermediate rows."
    }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) {
            return Vec::new();
        }
        if query.raw_upper().contains("JOIN") {
            return Vec::new();
        }
        PAT_PRESTO_CROSS
            .find(&query.raw)
            .map(|m| {
                vec![self.build_issue(
                    query,
                    "Implicit cross-join detected - use explicit JOIN with ON clause.",
                    m.as_str(),
                )]
            })
            .unwrap_or_default()
...
impl Rule for PrestoOrderByWithoutLimitRule {
    ...
    fn name(&self) -> &'static str {
        "ORDER BY Without LIMIT on Distributed Engine"
    }
    fn severity(&self) -> Severity {
        Severity::Medium
    }
    ...
    fn impact(&self) -> &'static str {
        "All rows are sent to the coordinator node for sorting, causing OOM."
    }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) {
            return Vec::new();
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        PAT_SPARK_BC
            ...
            .map(|m| {
                vec![self.build_issue(
                    query,
                    "BROADCAST hint detected - ensure table fits in executor memory.",
                    m.as_str(),
                )]
            })
            .unwrap_or_default()
...
static PAT_SPARK_UDF: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bWHERE\b.*\b(?:udf|UDF)\s*\(").unwrap());
impl Rule for SparkUdfInWhereRule {
    fn id(&self) -> &'static str {
        "PERF-SPARK-002"
    }
    fn name(&self) -> &'static str {
        "UDF in WHERE Prevents Pushdown"
    }
    fn severity(&self) -> Severity {
        Severity::Medium
    }
    ...
    fn impact(&self) -> &'static str {
        "Without pushdown, Spark reads the entire table from storage."
    }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) {
            return Vec::new();
        ...
        PAT_SPARK_UDF
            ...
            .map(|m| {
                vec![self.build_issue(
                    query,
                    "UDF in WHERE clause - prevents predicate pushdown to data source.",
                    m.as_str(),
                )]
            })
            .unwrap_or_default()
...
static PAT_SQLITE_WAL: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bPRAGMA\s+journal_mode\s*=\s*\w+").unwrap());
impl Rule for SqliteWalModeRule {
    ...
    fn impact(&self) -> &'static str {
        "Without WAL, any write locks the entire database file."
    }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) {
            return Vec::new();
        ...
        if let Some(m) = PAT_SQLITE_WAL.find(&query.raw) {
            ...
            if matched_upper.ends_with("WAL") {
                return Vec::new();
            }
            return vec![self.build_issue(
                query,
                "Non-WAL journal mode detected - consider WAL for concurrency.",
                m.as_str(),
            )];
        }
        Vec::new()
...
impl Rule for LikeWithoutCollateNocaseRule {
    ...
    fn impact(&self) -> &'static str {
        "Without COLLATE NOCASE on the column, LIKE cannot use indexes."
    }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        PAT_SQLITE_LIKE
            ...
            .map(|m| {
                vec![self.build_issue(
                    query,
                    "LIKE query detected - ensure column has COLLATE NOCASE for index usage.",
                    m.as_str(),
                )]
            })
            .unwrap_or_default()
...
impl Rule for DuckDBCopyWithoutFormatRule {
    ...
    fn name(&self) -> &'static str {
        "COPY Without FORMAT Specification"
    }
    fn severity(&self) -> Severity {
        Severity::Medium
    }
    ...
    fn impact(&self) -> &'static str {
        "Without FORMAT, DuckDB guesses from file extension, which can fail."
    }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) {
            return Vec::new();
        }
        if !PAT_DUCK_COPY.is_match(&query.raw) {
            return Vec::new();
        }
...
static PAT_DUCK_IN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bIN\s*\(\s*(?:[^()]*,\s*){9,}").unwrap());
impl Rule for DuckDBLargeInListRule {
    fn id(&self) -> &'static str {
        "PERF-DUCK-002"
    }
    fn name(&self) -> &'static str {
        "Large IN List - Use VALUES Table"
    }
    fn severity(&self) -> Severity {
        Severity::Low
    }
    ...
    fn category(&self) -> Option<Category> {
        Some(Category::PerfMemory)
    }
    fn dialects(&self) -> DialectSet {
        DialectSet::new(&["duckdb"])
    }
    fn impact(&self) -> &'static str {
        "Large IN lists are slower than VALUES table with semi-join."
    }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        PAT_DUCK_IN
            ...
            .map(|m| {
                vec![self.build_issue(
                    query,
                    "Large IN list detected - consider VALUES table expression.",
                    m.as_str(),
                )]
            })
            .unwrap_or_default()
...
impl Rule for MysqlGroupByImplicitSortRule {
    ...
    fn name(&self) -> &'static str {
        "GROUP BY Implicit Sort (Removed in MySQL 8.0)"
    }
    fn severity(&self) -> Severity {
        Severity::Low
    }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        if !PAT_MYSQL_GB.is_match(&query.raw) {
            return Vec::new();
        }
...
impl Rule for LargeObjectUnboundedRule {
    ...
    fn name(&self) -> &'static str {
        "Large Object Column in Non-Filtered Query"
    }
    fn severity(&self) -> Severity {
        Severity::Medium
    }
