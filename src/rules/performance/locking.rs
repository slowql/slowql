...
static PAT_LOCK_001: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\bWITH\s*\(\s*(TABLOCK|TABLOCKX|HOLDLOCK|XLOCK|PAGLOCK|ROWLOCK|UPDLOCK|SERIALIZABLE)\s*\)").unwrap()
});
impl Rule for TableLockHintRule {
    ...
    fn impact(&self) -> &'static str {
        "Table-level locks block ALL concurrent access to the table."
    }
...
static PAT_LOCK_002: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)(\bWITH\s*\(\s*(NOLOCK|READUNCOMMITTED)\s*\))|(\bREAD\s+UNCOMMITTED\b)|(\bSET\s+TRANSACTION\s+ISOLATION\s+LEVEL\s+READ\s+UNCOMMITTED\b)").unwrap()
});
impl Rule for ReadUncommittedHintRule {
    ...
    fn impact(&self) -> &'static str {
        "NOLOCK reads uncommitted data, can skip rows, read rows twice, or return phantom data."
    }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if !self.dialect_matches(query) {
            return Vec::new();
...
static PAT_LOCK_003: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?is)\bBEGIN\s+(TRAN|TRANSACTION)\b[\s\S]{500,}?\b(COMMIT|ROLLBACK)\b").unwrap()
});
        ...
        PAT_LOCK_003
            ...
            .map(|_| {
                vec![self.build_issue(
                    query,
                    "Potentially long-running transaction detected (500+ characters).",
...
impl Rule for MissingTransactionIsolationRule {
    ...
    fn impact(&self) -> &'static str {
        "Default isolation levels vary by database and configuration."
    }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        if has_begin && !upper.contains("ISOLATION LEVEL") {
            ...
            return vec![self.build_issue(
                query,
                "Transaction without explicit isolation level.",
                snip,
            )];
        }
        Vec::new()
...
impl Rule for OracleForUpdateWithoutNowaitRule {
    ...
    fn name(&self) -> &'static str {
        "SELECT FOR UPDATE Without NOWAIT (Oracle)"
    }
    fn severity(&self) -> Severity {
        Severity::Medium
    }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        if upper.contains("NOWAIT") || upper.contains("SKIP LOCKED") || upper.contains("WAIT") {
            return Vec::new();
        }
