...
static PAT_HINT_001: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\bOPTION\s*\(\s*(FORCE\s+ORDER|HASH\s+JOIN|MERGE\s+JOIN|LOOP\s+JOIN|FAST\s+\d+|RECOMPILE|OPTIMIZE\s+FOR|MAXDOP|QUERYTRACEON|USE\s+PLAN)\b").unwrap()
});
impl Rule for QueryOptimizerHintRule {
    ...
    fn impact(&self) -> &'static str {
        "Query hints freeze execution plans. As data grows, hinted plans become suboptimal."
    }
...
static PAT_HINT_002: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\b(FORCE\s+INDEX|USE\s+INDEX|IGNORE\s+INDEX|WITH\s*\(\s*INDEX\s*[=(])\b")
        .unwrap()
});
...
static PAT_HINT_003: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bOPTION\s*\([^)]*MAXDOP\s+\d+").unwrap());
impl Rule for ParallelQueryHintRule {
    ...
    fn impact(&self) -> &'static str {
        "MAXDOP hints override server-level parallelism."
    }
