...
impl Rule for LargeInClauseRule {
    ...
    fn impact(&self) -> &'static str {
        "Large IN clauses (100+ values) consume memory and bloat the plan cache."
    }
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        if let Some(m) = PAT_IN_CLAUSE.find(&query.raw) {
            ...
            if let Some(close) = after.find(')') {
                ...
                if inner.to_uppercase().contains("SELECT") {
                    return Vec::new();
                }
...
static PAT_TEMP: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)\bSELECT\b[^;]*\bINTO\s+[#@]\w+").unwrap());
impl Rule for UnboundedTempTableRule {
    fn id(&self) -> &'static str {
        "PERF-MEM-002"
    }
    fn name(&self) -> &'static str {
        "Unbounded Temp Table Creation"
    }
    fn severity(&self) -> Severity {
        Severity::Medium
    }
...
impl Rule for GroupByHighCardinalityRule {
    ...
    fn name(&self) -> &'static str {
        "GROUP BY on High-Cardinality Expression"
    }
    fn severity(&self) -> Severity {
        Severity::Low
    }
    ...
    fn impact(&self) -> &'static str {
        "Grouping by high-cardinality columns creates millions of groups, consuming massive memory."
    }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        for col in HIGH_CARD_COLS {
            if raw_lower.contains(&format!("group by {}", col))
                || raw_lower.contains(&format!("group by\n{}", col))
            {
                let msg = format!(
...
impl Rule for SelectIntoTempWithoutIndexRule {
    ...
    fn name(&self) -> &'static str {
        "SELECT INTO Temp Table Without Index"
    }
    fn severity(&self) -> Severity {
        Severity::Medium
    }
...
static PAT_CONV_JOIN: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(?i)(\bJOIN\b.*\bCONVERT\s*\()|(\bCAST\s*\(.*\bJOIN\b)").unwrap());
impl Rule for ImplicitConversionInJoinRule {
    fn id(&self) -> &'static str {
        "PERF-TSQL-003"
    }
    fn name(&self) -> &'static str {
        "Implicit Conversion in JOIN Predicate"
    }
    fn severity(&self) -> Severity {
        Severity::High
    }
    ...
    fn check(&self, query: &Query) -> Vec<Issue> {
        ...
        PAT_CONV_JOIN
            ...
            .map(|m| {
                let msg = format!(
                    "Explicit CONVERT/CAST in JOIN predicate - check for implicit conversion: {}",
                    m.as_str()
                );
...
static PAT_ORDER_SUB_NO_LIMIT: Lazy<Regex> =
...
impl Rule for OrderByWithoutLimitInSubqueryRule {
    ...
    fn name(&self) -> &'static str {
        "ORDER BY Without LIMIT in Subquery"
    }
    fn severity(&self) -> Severity {
        Severity::Low
    }
    ...
    fn impact(&self) -> &'static str {
        "Sorting without LIMIT in subqueries wastes resources."
    }
    fn check(&self, query: &Query) -> Vec<Issue> {
        if let Some(m) = PAT_ORDER_SUB_NO_LIMIT.find(&query.raw) {
            ...
            if !matched_upper.contains("LIMIT") && !matched_upper.contains("TOP") {
                return vec![self.build_issue(
                    query,
                    "ORDER BY in subquery without LIMIT is meaningless and wastes resources.",
                    m.as_str(),
                )];
            }
        }
